...
matern.sim <- function(args_list, B, ebreg_I, verbose = FALSE, seed = 1641){
  ...
  adj <- 10
    ...
    Csig <- matern_cov(D, range = sig_sp, nu = 0.5, sigma2 = 1, nugget = nugget)
    diag(Csig) <- diag(Csig) + nugget
    Rsig <- chol(Csig)
    Clat <- matern_cov(D, range = lat_sp, nu = 0.5, sigma2 = 1, nugget = nugget)
    diag(Clat) <- diag(Clat) + nugget
    Rlat <- chol(Clat)
  ...
  for(k in 1:K){
    ...
    set.seed(seed + k)
    data <- sim_LR_mat(x, N, M, M1, Rlat, Rsig, seed = seed + k)
    ...
    #### spatial all-at-once probe
    test3 <- system.time(mod.out.sp <- sp_probe(Y = Y, X = X, coord_mat, 
                                          verbose = TRUE, plot_ind = TRUE, eta_i = eta_i))
    
    alpha_est <- mod.out.sp$Calb_mod$coef[2]
    gamma_est <- mod.out.sp$E_step$gamma
    beta_est <- mod.out.sp$beta_hat
    beta_ast_est <- mod.out.sp$beta_ast_hat
    sigma2_est_new <- mod.out.sp$Calb_mod$sigma2_est
    Y_pred <- mod.out.sp$Calb_mod$Y_pred
    
    #Prediction for test data
    pred_res_test.sp <- predict_probe_func(mod.out.sp, X_test, Z = NULL, alpha = alpha)
    # Proportion of test PIs that contain the test observation
    ECP_PI <- mean(1*I(Y_test>pred_res_test.sp$PI_L & Y_test<pred_res_test.sp$PI_U))
    # Proportion of test CIs that contain the test true signal
    ECP_CI <- mean(1*I(eta_test>pred_res_test.sp$CI_L & eta_test<pred_res_test.sp$CI_U))
    
    PROBE_sp_res[k,] <- c(sum(gamma_est),
                       sum(gamma_est[signal]), 
                       mean((Y_pred-eta_i)^2), 
                       median(abs(Y_pred - eta_i)), ECP_PI, ECP_CI,   
                       mod.out.sp$count, sigma2_est_new, mean((Y_test - pred_res_test.sp$Pred)^2), 
                       mean((eta_test - pred_res_test.sp$Pred)^2), 
                       mean((eta_vec - beta_ast_est)^2),
                       mod.out.sp$conv, test3[3])
...
sim_LR_mat <- function(x, N, M, M1, Rlat, Rsig, seed=NULL){
  ...
  LP_data <- expand.grid(x1=x,
                         x2=x)
  D <- as.integer(sqrt(M))
  t_data <- simulate_matern_threshold(2*N, nrow = D, ncol = D,
                                      R = Rlat)
  signal_data <- simulate_matern_threshold(1, nrow = D, ncol = D,
                                      R = Rsig)
