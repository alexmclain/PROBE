...
foreach(set_i = settings_to_try)  %dopar% {
  ...
  sim1 <- block_simulation(args_list, B = 20, ebreg_I = FALSE, verbose = TRUE, seed = seed)
  
  sim2 <- block_simulation_robust(args_list, B = 20, ebreg_I = FALSE, verbose = TRUE, seed = seed)
}
gc()
print("Finished.")
